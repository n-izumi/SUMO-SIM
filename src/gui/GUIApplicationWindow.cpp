...
GUIApplicationWindow::buildToolBars() {
    ...
    {
        myToolBarDrag11 = new FXToolBarShell(this, GUIDesignToolBar);
        myToolBar11 = new FXToolBar(myTopDock, myToolBarDrag11, GUIDesignToolBarRaisedSameTop);
        new FXToolBarGrip(myToolBar11, myToolBar11, FXToolBar::ID_TOOLBARGRIP, GUIDesignToolBarGrip);
        new MFXButtonTooltip(myToolBar11, myStaticTooltipMenu, TL("現在時刻:\tToggle between time formats\tToggle between seconds and hour:minute:seconds display."), nullptr, this, MID_TIME_TOGGLE, GUIDesignButtonToolbarText);

        myCTSLabel = new MFXLCDLabel(myToolBar11, myStaticTooltipMenu, 16, nullptr, 0, JUSTIFY_RIGHT);
        myCTSLabel->setHorizontal(2);
        myCTSLabel->setVertical(6);
        myCTSLabel->setThickness(2);
        myCTSLabel->setGroove(2);
        myCTSLabel->setText("----------------");
    }
    // Simulation Step Display
    {
        ...
        new MFXButtonTooltip(myToolBar3, myStaticTooltipMenu, TL("SimTime:\tToggle between time formats\tToggle between seconds and hour:minute:seconds display."), nullptr, this, MID_TIME_TOGGLE, GUIDesignButtonToolbarText);
...
void
GUIApplicationWindow::updateTimeStr() {
    std::string timeStr = myRunThread->getNet().getCurrentTimeStr();
    myCTSLabel->setText(timeStr.c_str());
}
