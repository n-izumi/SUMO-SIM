...
int
Vehicle::getLicenseNumber(const std::string& vehID) {
    return Helper::getVehicle(vehID)->getLicenseNumber();
}
...
Vehicle::add(const std::string& vehID,
    ...
    if (licenseNumber != 0) {
        vehicleParams.parametersSet |= VEHPARS_LICENSE_NUMBER_SET;
    }
    // build vehicle
    ...
    vehicleParams.licenseNumber = licenseNumber;
...
Vehicle::handleVariable(const std::string& objID, const int variable, VariableWrapper* wrapper, tcpip::Storage* paramData) {
    switch (variable) {
        ...
        case VAR_LICENSE_NUMBER:
            return wrapper->wrapInt(objID, variable, getLicenseNumber(objID));
