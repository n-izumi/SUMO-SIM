...
std::vector<std::string>
LaneArea::getLaneIDs(const std::string& detID) {
    std::vector<std::string> res;
    std::vector<MSLane*>lanes = getDetector(detID)->getLanes();
    for (auto lane : lanes) {
        res.push_back(lane->getID());
    }
    return res;
}
...
double
LaneArea::getVehicleDistToDetectorEnd(const std::string& detID, const std::string& param) {
    return getDetector(detID)->getDistToDetectorEnd(param);
}
...
LaneArea::handleVariable(const std::string& objID, const int variable, VariableWrapper* wrapper, tcpip::Storage* paramData) {
    switch (variable) {
        ...
        case VAR_DIST_TO_DETECTOR_END:
            paramData->readUnsignedByte();
            return wrapper->wrapDouble(objID, variable, getVehicleDistToDetectorEnd(objID, paramData->readString()));
        case VAR_LENGTH:
