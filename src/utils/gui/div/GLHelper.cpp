...
GLHelper::drawText(const std::string& text, const Position& pos, const double layer, const double size,
                   const RGBColor& col, const double angle, const int align, double width, int fontSize) {
    if (width <= 0) {
        width = size;
    }
    if (!initFont()) {
    ...
    if (myGL2PSActive) {
        ...
        gl2psTextOptColor(text.c_str(), "Roboto", fontSize, align == 0 ? GL2PS_TEXT_C : align, (GLfloat) - angle, color);
        GLHelper::popMatrix();
        return;
    }
#endif
...
void
GLHelper::drawBox(const Position& pos, const double layer, const double size,
                      const RGBColor& bgColor, const RGBColor& borderColor,
                      const double angle,
                      const double relBorder,
    ...
    if (bgColor.alpha() != 0) {
        const double boxAngle = 90;
        const double borderWidth = size * relBorder;
        const double boxHeight = relHeight;
        const double boxWidth = relWidth;
        GLHelper::pushMatrix();
        glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
        glTranslated(pos.x(), pos.y(), layer);
        glRotated(-angle, 0, 0, 1);
