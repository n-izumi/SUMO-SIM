...
void
GUIGlObject::drawName(const Position& pos, const double scale, const GUIVisualizationTextSettings& settings, const double angle, bool forceShow, const std::string& text) const {
    if (settings.show(this) || forceShow) {
        std::string drawText = getMicrosimID();

        // vollmont
        if (text != "") {
            drawText = text;
        }
        GLHelper::drawTextSettings(settings, drawText, pos, scale, angle);
    }
}
